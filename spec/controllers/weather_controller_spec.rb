...
RSpec.describe WeatherController, type: :controller do
  ...
  describe "#show" do
    ...
    let(:filename) { "/mean_temp_#{date.to_s(:number)}.png" }
    before(:each) do
  ...
  describe "#all_for_date" do
    ...
    before(:each) do
      FactoryBot.create(:weather_datum, date: date)
      FactoryBot.create(:weather_data_import, readings_on: date)
      FactoryBot.create(:weather_datum, date: date2)
      FactoryBot.create(:weather_data_import, readings_on: date2)
    end
  ...
  describe "#info" do
    ...
    it "has the correct structure" do
      get :info
      expect(json.keys).to match([:date_range, :total_days, :lat_range, :long_range, :table_cols])
    end
